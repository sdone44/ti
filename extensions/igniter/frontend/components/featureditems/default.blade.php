                ...
                @foreach ($featuredMenuItems as $featuredItem)
                    <div class="col-sm-{{ round(12 / $featuredPerRow) }} mb-3 mb-sm-0">
                        <a
                            ...
                            href="{{page_url('local/menus', ['location' => isset($featuredItem->locations->first()->permalink_slug)??$featuredItem->locations->first()->permalink_slug])}}?menuId={{ $featuredItem->getBuyableIdentifier() }}"
                        >
